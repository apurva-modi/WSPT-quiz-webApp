<?php 
    $q = "select seen from student_view_status where uin = ". $_SESSION["uin"]. " and category_id = 555" ;
    $seen = $conn->query($q);
    if ($seen->num_rows > 0) {
...
<!-- Reading Reflection Begins here  -->
<div class="tab" >
    <div class="head justify-content-center">
        <h2>Reading Reflection</h2>
    </div>
    <?php 
            $question_id = 0;
            $sql = "SELECT * FROM `questions` where `category_id` = 555";
            ...
            if ($questions->num_rows > 0) {
                ...
                while($question_rows = $questions->fetch_assoc()) {
                    if($question_rows["question_type"]=="Short Answer")
                    {
                                    ...
                                    <div class=\"col\">
                                        ...
                                        <div class=\" \">
                                            <p><input placeholder=\"Short Answer\" 
                                                      ...
                                                      name=\"RR_Question".$question_rows["id"]."\">
                                            </p>
                                        </div>
                                        ";
                        echo $str;
                    ...
                    if($question_rows["question_type"]=="Select")
                    {
                        ...
                        if ($options->num_rows > 0) {
                            ...
                            while($option_row = $options->fetch_assoc()) {
                                ...
                                <input type=\"radio\"  class=\"custom-control-input\" id=\"RR_Question".$option_row["id"].
                                "\" name=\"RR_Question".$option_row["question_id"]."\" required=\"required\"><label class=\"custom-control-label\" for=\"RR_Question".$option_row["id"]."\">".$option_row["options_content"]."</label></div>";
                                echo $str;
