<?php 
    $q = "select seen from student_view_status where uin = ". $_SESSION["uin"]. " and category_id = 777" ;
    $seen = $conn->query($q);
    if ($seen->num_rows > 0) {
...
<!-- Grammar Reflection Begins here  -->
<div class="tab" >
    <div class="head justify-content-center">
        <h2>Grammar Reflection</h2>
    </div>

        <?php 
            $question_id = 0;
            $sql = "SELECT * FROM `questions` where `category_id` = 777 ORDER BY `id`";
                    ...
                    while($option_row = $options->fetch_assoc()) {
                        $str = "<div class=\"custom-control custom-radio\">
                                    <input type=\"radio\"  
                                           ...
                                           id=\"GR_Question" .$option_row["id"]. "\"
                                           name=\"GR_Question" .$option_row["question_id"]. "\" 
                                           required=\"required\">
                                                <label class=\"custom-control-label\" 
                                                    for=\"GR_Question" .$option_row["id"]. "\">"
                                                    .$option_row["options_content"].
                                                "</label>
                                </div>";
                        echo $str;
                    }
